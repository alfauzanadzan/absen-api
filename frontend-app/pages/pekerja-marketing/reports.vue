    ...
    <div class="flex">
      ...
      <aside class="w-60 bg-white p-8 flex flex-col">
        <div class="flex items-center justify-center h-20 mb-6">
          <h2 class="text-xl font-bold text-blue-600">Marketing</h2>
        </div>

        <nav class="flex flex-col space-y-2 text-gray-700">
          <NuxtLink to="/pekerja-marketing/pekerjamarketing" class="p-2 rounded hover:bg-gray-100">Dashboard</NuxtLink>
          <NuxtLink to="/pekerja-marketing/profilpekerja" class="p-2 rounded hover:bg-gray-100">Profile</NuxtLink>
          <NuxtLink to="/pekerja-marketing/reports" class="p-2 rounded bg-blue-50 text-blue-600 font-medium">Reports</NuxtLink>
        </nav>
      </aside>
            ...
            <div class="flex items-center gap-3">
              ...
              >
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none">
                  ...
                  <path d="M21 15v4a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1v-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                Import PDF
              </button>
              ...
              <div class="md:col-span-2 flex items-center gap-2">
                <div>
                  ...
                  <div class="flex items-center gap-2">
                    <button @click="changeMonth(-1)" class="p-2 border rounded hover:bg-gray-100" aria-label="prev month">‹</button>
                    <select v-model="selectedMonth" class="border rounded px-3 py-2">
                      <option v-for="(m, idx) in months" :key="idx" :value="idx">{{ m }}</option>
                    </select>
                    <button @click="changeMonth(1)" class="p-2 border rounded hover:bg-gray-100" aria-label="next month">›</button>
                  </div>
                </div>
              </div>
              ...
              <tbody>
                <tr v-for="rec in filteredRecords" :key="rec.date" class="odd:bg-white even:bg-gray-50 border-t">
                  ...
                  <td class="px-6 py-4 text-sm">
                    <span :class="statusClass(rec.status)" class="font-semibold">{{ rec.status }}</span>
                  </td>
                </tr>
...
function importPdf() {
  alert('Import PDF — implementasikan sesuai backend / parsing Anda.')
}
