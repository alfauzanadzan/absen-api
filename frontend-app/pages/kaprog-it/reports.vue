  ...
  <div class="flex h-screen bg-gradient-to-br from-gray-400 via-gray-300 to-gray-500">
    ...
    <aside class="w-64 bg-white/30 backdrop-blur-md p-6 flex flex-col shadow-lg border-r border-white/30">
      ...
      <nav class="flex flex-col space-y-3 text-white font-medium">
        ...
        <a href="/kaprog-it/reports" class="p-3 rounded-lg bg-white/30 text-white shadow hover:bg-white/40 transition">📊 Reports</a>
      </nav>
    </aside>
    ...
    >
      <div class="bg-white rounded-lg w-11/12 md:w-2/3 p-6 shadow-lg">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold">Employee List (IT)</h3>
...
const fetchEmployees = async () => {
  try {
    ...
    // ✅ Filter hanya pekerja dari departemen IT
    const filtered = data.filter(
      (u: any) =>
        u.role === "PEKERJA" &&
        (u.department?.toLowerCase?.() === "it" ||
          u.departmentName?.toLowerCase?.() === "it")
    )
