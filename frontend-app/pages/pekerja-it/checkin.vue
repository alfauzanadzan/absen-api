...
const handleDecodedRaw = async (raw: string) => {
  ...
  // ✅ Pastikan hanya PEKERJA IT yang bisa absen
  if (user.value.departmentName?.toLowerCase() !== "it") {
    message.value = "❌ Hanya pekerja IT yang bisa absen di halaman ini"
    debounceLock = false
    return
  }
  ...
  try {
    ...
    const qrDept = qrData?.departmentName?.toLowerCase?.() || ""

    // 🚫 Kalau QR bukan milik department IT
    if (qrDept !== "it") {
      message.value = "❌ QR Code tidak sesuai department (IT)"
      debounceLock = false
      return
    }
  ...
  <div class="flex h-screen bg-gray-50">
    ...
    <aside class="w-60 bg-white p-6 flex flex-col">
      <div class="flex items-center justify-center h-20 mb-6">
        <h1 class="text-lg font-bold text-purple-600">PEKERJA IT</h1>
      </div>
      <nav class="flex flex-col space-y-2">
        <a href="/pekerja-it/pekerjait" class="p-2 rounded hover:bg-gray-400">🏠 Dashboard</a>
        <a href="/pekerja-it/checkin" class="p-2 rounded bg-purple-50 text-purple-600 font-medium">🕓 Check-in</a>
        <a href="/pekerja-it/checkout" class="p-2 rounded hover:bg-gray-400">⏰ Check-out</a>
      </nav>
    </aside>
      ...
      <div class="w-full max-w-2xl">
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-sm text-gray-500">Arahkan kamera ke QR Code IT Department</p>
          ...
          <div class="w-80 h-80 bg-black rounded overflow-hidden relative shadow">
            ...
            <div class="absolute left-0 right-0 bottom-0 p-3 bg-black/40 text-white flex items-center justify-between text-sm">
              ...
              <div>
                ...
                <button v-else @click="startScanner" class="px-3 py-1 bg-purple-500 rounded text-xs">Start</button>
              </div>
            </div>
          </div>
