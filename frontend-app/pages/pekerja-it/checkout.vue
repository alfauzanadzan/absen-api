...
const postCheckout = async (payload: { userId: string; role: string; qrValue: string }) => {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      else if (user.value?.role === 'PEKERJA') router.push('/pekerja-it/pekerjait')
...
const handleDecodedRaw = async (raw: string) => {
  ...
  try {
    ...
    // 🔒 Validasi barcode sesuai department
  ...
  <div class="flex h-screen bg-gray-50">
    ...
    <aside class="w-60 bg-white p-6 flex flex-col">
      ...
      <nav class="flex flex-col space-y-2">
        ...
        <a href="/pekerja-it/checkin" class="p-2 rounded hover:bg-gray-400">🕓 Check-in</a>
        <a href="/pekerja-it/checkout" class="p-2 rounded bg-blue-50 text-blue-600 font-medium">⏰ Check-out</a>
      </nav>
    </aside>
